...
# ROS2 APP: velocity_offboard_test (velocity offboard control example).
# This only RUNS the node. Build it beforehand with:
#   ./scripts/run.sh ros2 build ros2_ws velocity_offboard_test
...
if [ ! -d "${ROS2_WS}/install/velocity_offboard_test" ]; then
    echo "[$(basename "$0")] ERROR: velocity_offboard_test is not built."
    echo "[$(basename "$0")] Build it first: ./scripts/run.sh ros2 build ros2_ws velocity_offboard_test"
    exit 1
fi
